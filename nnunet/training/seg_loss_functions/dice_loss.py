...
import torch
from nnunet.training.seg_loss_functions.ND_Crossentropy import CrossentropyND, TopKLoss, WeightedCrossEntropyLoss
from nnunet.training.seg_loss_functions.focal_loss import FocalLoss
...
class DC_and_focal_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, focal_kwargs, aggregate="sum"):
        ...
        self.aggregate = aggregate
        self.ce = FocalLoss(apply_nonlin=softmax_helper, alpha=0.5, gamma=2, smooth=1e-5, **focal_kwargs)
        self.dc = SoftDiceLoss(apply_nonlin=softmax_helper, **soft_dice_kwargs)
